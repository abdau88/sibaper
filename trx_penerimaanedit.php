...
// Create page object
$trx_penerimaan_edit = new trx_penerimaan_edit();

// Run the page
$trx_penerimaan_edit->run();
...
$trx_penerimaan_edit->Page_Render();
?>
...
var ftrx_penerimaanedit = currentForm = new ew.Form("ftrx_penerimaanedit", "edit");

// Validate form
ftrx_penerimaanedit.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($trx_penerimaan_edit->tgl_penerimaan->Required) { ?>
		...
		<?php if ($trx_penerimaan_edit->kd_penerima->Required) { ?>
		...
		<?php if ($trx_penerimaan_edit->kd_unit->Required) { ?>
...
ftrx_penerimaanedit.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
ftrx_penerimaanedit.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftrx_penerimaanedit.lists["x_kd_penerima"] = <?php echo $trx_penerimaan_edit->kd_penerima->Lookup->toClientList() ?>;
ftrx_penerimaanedit.lists["x_kd_penerima"].options = <?php echo JsonEncode($trx_penerimaan_edit->kd_penerima->lookupOptions()) ?>;
ftrx_penerimaanedit.autoSuggests["x_kd_penerima"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
ftrx_penerimaanedit.lists["x_kd_unit"] = <?php echo $trx_penerimaan_edit->kd_unit->Lookup->toClientList() ?>;
ftrx_penerimaanedit.lists["x_kd_unit"].options = <?php echo JsonEncode($trx_penerimaan_edit->kd_unit->lookupOptions()) ?>;
...
<?php $trx_penerimaan_edit->showPageHeader(); ?>
<?php
$trx_penerimaan_edit->showMessage();
?>
<form name="ftrx_penerimaanedit" id="ftrx_penerimaanedit" class="<?php echo $trx_penerimaan_edit->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($trx_penerimaan_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $trx_penerimaan_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="trx_penerimaan">
<input type="hidden" name="action" id="action" value="update">
<input type="hidden" name="modal" value="<?php echo (int)$trx_penerimaan_edit->IsModal ?>">
<div class="ew-edit-div"><!-- page* -->
<?php if ($trx_penerimaan->tgl_penerimaan->Visible) { // tgl_penerimaan ?>
	<div id="r_tgl_penerimaan" class="form-group row">
		<label id="elh_trx_penerimaan_tgl_penerimaan" for="x_tgl_penerimaan" class="<?php echo $trx_penerimaan_edit->LeftColumnClass ?>"><?php echo $trx_penerimaan->tgl_penerimaan->caption() ?><?php echo ($trx_penerimaan->tgl_penerimaan->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $trx_penerimaan_edit->RightColumnClass ?>"><div<?php echo $trx_penerimaan->tgl_penerimaan->cellAttributes() ?>>
...
<script>
ew.createDateTimePicker("ftrx_penerimaanedit", "x_tgl_penerimaan", {"ignoreReadonly":true,"useCurrent":false,"format":5});
</script>
<?php } ?>
</span>
...
<?php if ($trx_penerimaan->kd_penerima->Visible) { // kd_penerima ?>
	<div id="r_kd_penerima" class="form-group row">
		<label id="elh_trx_penerimaan_kd_penerima" class="<?php echo $trx_penerimaan_edit->LeftColumnClass ?>"><?php echo $trx_penerimaan->kd_penerima->caption() ?><?php echo ($trx_penerimaan->kd_penerima->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $trx_penerimaan_edit->RightColumnClass ?>"><div<?php echo $trx_penerimaan->kd_penerima->cellAttributes() ?>>
...
<script>
ftrx_penerimaanedit.createAutoSuggest({"id":"x_kd_penerima","forceSelect":false});
</script>
...
<?php if ($trx_penerimaan->kd_unit->Visible) { // kd_unit ?>
	<div id="r_kd_unit" class="form-group row">
		<label id="elh_trx_penerimaan_kd_unit" for="x_kd_unit" class="<?php echo $trx_penerimaan_edit->LeftColumnClass ?>"><?php echo $trx_penerimaan->kd_unit->caption() ?><?php echo ($trx_penerimaan->kd_unit->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $trx_penerimaan_edit->RightColumnClass ?>"><div<?php echo $trx_penerimaan->kd_unit->cellAttributes() ?>>
...
<?php } ?>
	...
	<input type="hidden" data-table="trx_penerimaan" data-field="x_kd_penerimaan" name="x_kd_penerimaan" id="x_kd_penerimaan" value="<?php echo HtmlEncode($trx_penerimaan->kd_penerimaan->CurrentValue) ?>">
<?php
	if (in_array("trx_penerimaan_detail", explode(",", $trx_penerimaan->getCurrentDetailTable())) && $trx_penerimaan_detail->DetailEdit) {
?>
...
<?php } ?>
<?php if (!$trx_penerimaan_edit->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $trx_penerimaan_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->phrase("SaveBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $trx_penerimaan_edit->getReturnUrl() ?>"><?php echo $Language->phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$trx_penerimaan_edit->showPageFooter();
if (DEBUG_ENABLED)
