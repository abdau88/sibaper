...
/**
 * Table class for v_lap_pembelian
 */
class v_lap_pembelian extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'v_lap_pembelian';
		$this->TableName = 'v_lap_pembelian';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`v_lap_pembelian`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// tgl_pembelian
		$this->tgl_pembelian = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_tgl_pembelian', 'tgl_pembelian', '`tgl_pembelian`', CastDateFieldForLike('`tgl_pembelian`', 0, "DB"), 133, 0, FALSE, '`tgl_pembelian`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tgl_pembelian->Sortable = TRUE; // Allow sort
		$this->tgl_pembelian->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['tgl_pembelian'] = &$this->tgl_pembelian;

		// kode_barang
		$this->kode_barang = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_kode_barang', 'kode_barang', '`kode_barang`', '`kode_barang`', 200, -1, FALSE, '`kode_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// nama_barang
		$this->nama_barang = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_nama_barang', 'nama_barang', '`nama_barang`', '`nama_barang`', 200, -1, FALSE, '`nama_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_barang->Sortable = TRUE; // Allow sort
		$this->fields['nama_barang'] = &$this->nama_barang;

		// nama_kategori
		$this->nama_kategori = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_nama_kategori', 'nama_kategori', '`nama_kategori`', '`nama_kategori`', 200, -1, FALSE, '`nama_kategori`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_kategori->Sortable = TRUE; // Allow sort
		$this->fields['nama_kategori'] = &$this->nama_kategori;

		// nama_satuan
		$this->nama_satuan = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_nama_satuan', 'nama_satuan', '`nama_satuan`', '`nama_satuan`', 200, -1, FALSE, '`nama_satuan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_satuan->Sortable = TRUE; // Allow sort
		$this->fields['nama_satuan'] = &$this->nama_satuan;

		// nama_vendor
		$this->nama_vendor = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_nama_vendor', 'nama_vendor', '`nama_vendor`', '`nama_vendor`', 200, -1, FALSE, '`nama_vendor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_vendor->Sortable = TRUE; // Allow sort
		$this->fields['nama_vendor'] = &$this->nama_vendor;

		// qty
		$this->qty = new DbField('v_lap_pembelian', 'v_lap_pembelian', 'x_qty', 'qty', '`qty`', '`qty`', 3, -1, FALSE, '`qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`v_lap_pembelian`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->nama_vendor->DbValue = $row['nama_vendor'];
	...
	{
		return "`kode_barang` = '@kode_barang@'";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "v_lap_pembelianlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "v_lap_pembelianview.php")
			return $Language->phrase("View");
		elseif ($pageName == "v_lap_pembelianedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "v_lap_pembelianadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("v_lap_pembelianview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("v_lap_pembelianview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "v_lap_pembelianadd.php?" . $this->getUrlParm($parm);
		else
			$url = "v_lap_pembelianadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_lap_pembelianedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("v_lap_pembelianadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("v_lap_pembeliandelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys()
	{
		...
		foreach ($arKeys as $key) {
			...
			$this->kode_barang->CurrentValue = $key;
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->nama_vendor->setDbValue($rs->fields('nama_vendor'));
	...
	public function renderEditRow()
	{
		...
		$this->tgl_pembelian->EditValue = FormatDateTime($this->tgl_pembelian->CurrentValue, 8);
		...
		// nama_vendor
		$this->nama_vendor->EditAttrs["class"] = "form-control";
		$this->nama_vendor->EditCustomAttributes = "";
		if (REMOVE_XSS)
			$this->nama_vendor->CurrentValue = HtmlDecode($this->nama_vendor->CurrentValue);
		$this->nama_vendor->EditValue = $this->nama_vendor->CurrentValue;
		$this->nama_vendor->PlaceHolder = RemoveHtml($this->nama_vendor->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->nama_vendor);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->nama_vendor);
