...
/**
 * Table class for trx_pembelian_detail
 */
class trx_pembelian_detail extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'trx_pembelian_detail';
		$this->TableName = 'trx_pembelian_detail';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`trx_pembelian_detail`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// no
		$this->no = new DbField('trx_pembelian_detail', 'trx_pembelian_detail', 'x_no', 'no', '`no`', '`no`', 3, -1, FALSE, '`no`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// kode_pembelian
		$this->kode_pembelian = new DbField('trx_pembelian_detail', 'trx_pembelian_detail', 'x_kode_pembelian', 'kode_pembelian', '`kode_pembelian`', '`kode_pembelian`', 3, -1, FALSE, '`kode_pembelian`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// nama_barang
		$this->nama_barang = new DbField('trx_pembelian_detail', 'trx_pembelian_detail', 'x_nama_barang', 'nama_barang', '`nama_barang`', '`nama_barang`', 200, -1, FALSE, '`nama_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// qty
		$this->qty = new DbField('trx_pembelian_detail', 'trx_pembelian_detail', 'x_qty', 'qty', '`qty`', '`qty`', 3, -1, FALSE, '`qty`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->qty->Sortable = TRUE; // Allow sort
		$this->qty->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['qty'] = &$this->qty;

		// kd_satuan
		$this->kd_satuan = new DbField('trx_pembelian_detail', 'trx_pembelian_detail', 'x_kd_satuan', 'kd_satuan', '`kd_satuan`', '`kd_satuan`', 3, -1, FALSE, '`kd_satuan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kd_satuan->Sortable = TRUE; // Allow sort
		$this->kd_satuan->Lookup = new Lookup('kd_satuan', 'tb_satuan', FALSE, 'kd_satuan', ["nama_satuan","","",""], [], [], [], [], [], [], '', '');
		$this->kd_satuan->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['kd_satuan'] = &$this->kd_satuan;
	}
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "trx_pembelian") {
			if ($this->kode_pembelian->getSessionValue() <> "")
				$masterFilter .= "`kode_pembelian`=" . QuotedValue($this->kode_pembelian->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "trx_pembelian") {
			if ($this->kode_pembelian->getSessionValue() <> "")
				$detailFilter .= "`kode_pembelian`=" . QuotedValue($this->kode_pembelian->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_trx_pembelian()
	{
		return "`kode_pembelian`=@kode_pembelian@";
	}

	// Detail filter
	public function sqlDetailFilter_trx_pembelian()
	{
		return "`kode_pembelian`=@kode_pembelian@";
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`trx_pembelian_detail`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->kd_satuan->DbValue = $row['kd_satuan'];
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "trx_pembelian_detaillist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "trx_pembelian_detailview.php")
			return $Language->phrase("View");
		elseif ($pageName == "trx_pembelian_detailedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "trx_pembelian_detailadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("trx_pembelian_detailview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("trx_pembelian_detailview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "trx_pembelian_detailadd.php?" . $this->getUrlParm($parm);
		else
			$url = "trx_pembelian_detailadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("trx_pembelian_detailedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("trx_pembelian_detailadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("trx_pembelian_detaildelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "trx_pembelian" && !ContainsString($url, TABLE_SHOW_MASTER . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_kode_pembelian=" . urlencode($this->kode_pembelian->CurrentValue);
		}
		return $url;
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->kd_satuan->setDbValue($rs->fields('kd_satuan'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->kode_pembelian->PlaceHolder = RemoveHtml($this->kode_pembelian->caption());
		}
		...
		$this->kd_satuan->EditValue = $this->kd_satuan->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->kd_satuan);
				} else {
					$doc->exportCaption($this->no);
					$doc->exportCaption($this->kode_pembelian);
					$doc->exportCaption($this->nama_barang);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->kd_satuan);
					} else {
						$doc->exportField($this->no);
						$doc->exportField($this->kode_pembelian);
						$doc->exportField($this->nama_barang);
