 ...
 */
class tb_barang extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// kode_barang
		$this->kode_barang = new DbField('tb_barang', 'tb_barang', 'x_kode_barang', 'kode_barang', '`kode_barang`', '`kode_barang`', 200, -1, FALSE, '`kode_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// nama_barang
		$this->nama_barang = new DbField('tb_barang', 'tb_barang', 'x_nama_barang', 'nama_barang', '`nama_barang`', '`nama_barang`', 200, -1, FALSE, '`nama_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama_barang->Sortable = TRUE; // Allow sort
		$this->fields['nama_barang'] = &$this->nama_barang;

		// kd_kategori
		$this->kd_kategori = new DbField('tb_barang', 'tb_barang', 'x_kd_kategori', 'kd_kategori', '`kd_kategori`', '`kd_kategori`', 3, -1, FALSE, '`kd_kategori`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->kd_kategori->Sortable = TRUE; // Allow sort
		$this->kd_kategori->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->kd_kategori->PleaseSelectText = $Language->phrase("PleaseSelect"); // PleaseSelect text
		$this->kd_kategori->Lookup = new Lookup('kd_kategori', 'tb_kategori', FALSE, 'kd_kategori', ["nama_kategori","","",""], [], [], [], [], [], [], '', '');
		$this->kd_kategori->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['kd_kategori'] = &$this->kd_kategori;

		// kd_satuan
		$this->kd_satuan = new DbField('tb_barang', 'tb_barang', 'x_kd_satuan', 'kd_satuan', '`kd_satuan`', '`kd_satuan`', 3, -1, FALSE, '`kd_satuan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->kd_satuan->Sortable = TRUE; // Allow sort
		$this->kd_satuan->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->kd_satuan->PleaseSelectText = $Language->phrase("PleaseSelect"); // PleaseSelect text
		...
		// stok_awal
		$this->stok_awal = new DbField('tb_barang', 'tb_barang', 'x_stok_awal', 'stok_awal', '`stok_awal`', '`stok_awal`', 3, -1, FALSE, '`stok_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->stok_awal->Nullable = FALSE; // NOT NULL field
		$this->stok_awal->Required = TRUE; // Required field
		$this->stok_awal->Sortable = TRUE; // Allow sort
		$this->stok_awal->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['stok_awal'] = &$this->stok_awal;

		// stok_akhir
		$this->stok_akhir = new DbField('tb_barang', 'tb_barang', 'x_stok_akhir', 'stok_akhir', '`stok_akhir`', '`stok_akhir`', 3, -1, FALSE, '`stok_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`tb_barang`";
	}
	...
	{
		return ($this->SqlOrderBy <> "") ? $this->SqlOrderBy : "`nama_barang` ASC";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->kd_kategori->DbValue = $row['kd_kategori'];
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "tb_barangedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "tb_barangadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("tb_barangview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("tb_barangview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "tb_barangadd.php?" . $this->getUrlParm($parm);
		else
	...
	{
		$url = $this->keyUrl("tb_barangedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("tb_barangadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("tb_barangdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->kd_kategori->setDbValue($rs->fields('kd_kategori'));
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->kd_kategori);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->kd_kategori);
